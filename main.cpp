...
void createObjects(void){
    ...
    objects[0].set(0, -1, 0, 100, 1, 1, 1, 1, 1, 1);
    
    objects[1].set(3,0, 0, 1, 1.5, 1, 1, 1, 1, 1);
    
    objects[2].set(1, 5, -25, 20, 20, 20, 0,1,0.5, 2);
    
    objects[3].set(5, 5, -25, 2, 2, 2, 0,0.4,0.5, 3);
    
    objects[4].set(10, 3, -20, 10, 10, 10, 0,0.4,0.5, 1);
...
void init(void)
{
	...
	snail_tex = LoadPPM("test_c.ppm", &width, &height, &max);
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_STATIC_DRAW);
	...
	marble_tex = LoadPPM("marble.ppm", &width, &height, &max);
